...
import { StyleSheet, Text, View, TouchableOpacity, ActivityIndicator, ScrollView, useColorScheme, Image } from 'react-native';
import { User, ShieldCheck, LogOut, Package, Store, ChevronRight, List, TrendingUp, Bookmark, Globe, Settings } from 'lucide-react-native';
    ...
    return (
      <View style={[styles.unauthContainer, { backgroundColor: colors.backgroundSecondary }]}>
        <View style={styles.responsiveWrapper}>
          <View style={[styles.unauthIconContainer, { backgroundColor: colors.border }]}><User size={64} color={colors.textSecondary} /></View>
      ...
      <View style={styles.responsiveWrapper}>
        <View style={[styles.header, { backgroundColor: colors.card }]}>
          {profile?.avatar_url ? (
            ...
            <View style={styles.avatarPlaceholder}><Text style={styles.avatarText}>{profile?.first_name ? profile.first_name[0].toUpperCase() : 'U'}</Text></View>
          )}
        ...
        {(!profile || profile.tier === 'buyer') && (
          <View style={[styles.card, { backgroundColor: colors.card }]}>
            <View style={styles.cardHeader}><ShieldCheck size={24} color="#FF5A00" /><Text style={[styles.cardTitle, { color: colors.text }]}>{t('profile.want_to_sell')}</Text></View>
        ...
        {profile?.tier === 'private_seller' && (
          <>
            <View style={[styles.card, { backgroundColor: colors.card }]}>
              <View style={styles.cardHeader}><Package size={24} color="#FF5A00" /><Text style={[styles.cardTitle, { color: colors.text }]}>{t('profile.sales_management')}</Text></View>
              <View style={styles.actionRow}>
                <TouchableOpacity style={[styles.actionButton, { flex: 1, marginRight: 8 }]} onPress={() => router.push('/add-product')}><Text style={styles.actionButtonText}>{t('profile.add_product')}</Text></TouchableOpacity>
                <TouchableOpacity style={[styles.actionButtonSecondary, { flex: 1, marginLeft: 8, backgroundColor: colorScheme === 'dark' ? '#331200' : '#FFF5F0' }]} onPress={() => router.push('/my-products')}><List size={20} color="#FF5A00" style={{ marginRight: 8 }} /><Text style={styles.actionButtonTextSecondary}>{t('profile.my_products')}</Text></TouchableOpacity>
              </View>
            </View>
            <TouchableOpacity style={styles.businessPromoCard} onPress={() => router.push('/upgrade-business')}>
              <View style={styles.businessPromoContent}>
                ...
                <View style={styles.businessPromoTextContainer}><Text style={styles.businessPromoTitle}>{t('profile.become_business')}</Text><Text style={styles.businessPromoSubtitle}>{t('profile.remove_limits')}</Text></View>
              </View>
        ...
        {profile?.tier === 'business' && (
          <View style={[styles.card, { backgroundColor: colors.card }]}>
            <View style={styles.cardHeader}><Store size={24} color="#FF5A00" /><Text style={[styles.cardTitle, { color: colors.text }]}>{t('profile.store_panel')}</Text></View>
            <View style={styles.actionRow}>
              <TouchableOpacity style={[styles.actionButton, { flex: 1, marginRight: 8 }]} onPress={() => router.push('/add-product')}><Text style={styles.actionButtonText}>{t('profile.add_product')}</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.actionButtonSecondary, { flex: 1, marginLeft: 8, backgroundColor: colorScheme === 'dark' ? '#331200' : '#FFF5F0' }]} onPress={() => router.push('/my-products')}><List size={20} color="#FF5A00" style={{ marginRight: 8 }} /><Text style={styles.actionButtonTextSecondary}>{t('profile.my_products')}</Text></TouchableOpacity>
            </View>
            <TouchableOpacity style={[styles.analyticsButton, { backgroundColor: colors.backgroundSecondary }]}><TrendingUp size={20} color="#34C759" style={{ marginRight: 8 }} /><Text style={[styles.analyticsButtonText, { color: colors.text }]}>{t('profile.sales_stats')}</Text></TouchableOpacity>
          </View>
        )}

        <View style={[styles.menuSection, { backgroundColor: colors.card }]}>
          <TouchableOpacity style={[styles.menuItem, { borderBottomColor: colors.border }]} onPress={() => router.push('/my-orders')}><View style={styles.menuItemLeft}><Package size={20} color={colors.icon} /><Text style={[styles.menuItemText, { color: colors.text }]}>{t('profile.my_orders')}</Text></View><ChevronRight size={20} color={colors.textSecondary} /></TouchableOpacity>
          <TouchableOpacity style={[styles.menuItem, { borderBottomColor: colors.border }]} onPress={() => router.push('/favorites')}><View style={styles.menuItemLeft}><Bookmark size={20} color={colors.icon} /><Text style={[styles.menuItemText, { color: colors.text }]}>{t('profile.favorites')}</Text></View><ChevronRight size={20} color={colors.textSecondary} /></TouchableOpacity>
          <TouchableOpacity style={[styles.menuItem, { borderBottomColor: colors.border }]} onPress={toggleLanguage}><View style={styles.menuItemLeft}><Globe size={20} color={colors.icon} /><Text style={[styles.menuItemText, { color: colors.text }]}>{t('profile.language')}</Text></View><View style={styles.langBadge}><Text style={styles.langBadgeText}>{locale.toUpperCase()}</Text></View></TouchableOpacity>
        </View>

        <TouchableOpacity style={[styles.logoutButton, { backgroundColor: colorScheme === 'dark' ? '#330000' : '#FFF0F0' }]} onPress={signOut}><LogOut size={20} color="#FF3B30" /><Text style={styles.logoutText}>{t('profile.logout')}</Text></TouchableOpacity>
