...
import { 
  ...
  Image, SafeAreaView, Dimensions, FlatList, Share, Platform
} from 'react-native';
...
export default function ProductScreen() {
  ...
  const [liked, setLiked] = useState(false); // Локальный стейт для лайка (сердечка)
    ...
    const fetchProductDetails = async () => {
      try {
        const { data, error } = await supabase
          .from('products')
          .select(`*, profiles (id, first_name, last_name, tier, avatar_url)`)
          .eq('id', id)
          .single();
      ...
      if (Platform.OS === 'web') {
        try {
          ...
          Toast.show({
            type: 'success',
            text1: 'Скопировано',
            text2: 'Текст скопирован в буфер обмена',
            position: 'top'
          });
  ...
  const handleAddComment = () => {
    Toast.show({
      type: 'info',
      text1: 'В разработке',
      text2: 'Функция добавления отзывов появится в следующем обновлении.',
      position: 'top'
    });
  };
            ...
            <View key={comment.id} style={styles.commentCard}>
              ...
              <View style={styles.commentContent}>
                ...
                <View style={styles.starsRow}>
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={12} color={i < comment.rating ? "#FF9500" : "#E5E5EA"} fill={i < comment.rating ? "#FF9500" : "transparent"} />
                  ))}
                </View>
