...
import { 
  ...
  FlatList, SafeAreaView, ActivityIndicator, Image, Dimensions, Keyboard
} from 'react-native';
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                <SearchIcon size={48} color="#D1D1D6" />
                <Text style={styles.emptyTitle}>Ничего не найдено</Text>
                <Text style={styles.emptySubtitle}>Попробуйте изменить запрос</Text>
              </View>
            }
          />
        )
        ...
        <View style={styles.suggestionsContainer}>
          {recentSearches.length > 0 && (
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>Вы искали</Text>
              ...
              {recentSearches.map((item, index) => (
                <TouchableOpacity 
                  key={index} style={styles.recentItem}
                  onPress={() => { setQuery(item); performSearch(item); Keyboard.dismiss(); }}
                >
                  <Clock size={20} color="#8E8E93" />
                  <Text style={styles.recentText}>{item}</Text>
                  <ArrowLeft size={20} color="#E5E5EA" style={{ transform: [{ rotate: '135deg' }] }} />
                </TouchableOpacity>
              ))}
            </View>
          )}
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              ...
              <TrendingUp size={20} color="#FF5A00" />
            </View>
            <View style={styles.tagsContainer}>
              {POPULAR_CATEGORIES.map((cat, index) => (
                <TouchableOpacity 
                  key={index} style={styles.tag}
                  onPress={() => { setQuery(cat); performSearch(cat); Keyboard.dismiss(); }}
                >
...
const styles = StyleSheet.create({
  ...
  card: { width: COLUMN_WIDTH, backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#F2F2F7', overflow: 'hidden' },
  ...
  emptyTitle: { fontSize: 18, fontWeight: 'bold', color: '#1C1C1E', marginTop: 16, marginBottom: 8 },
