...
import { View, Text, StyleSheet, Image, TouchableOpacity, FlatList, SafeAreaView, ActivityIndicator, Dimensions } from 'react-native';
...
export default function StorefrontScreen() {
  ...
  const renderProduct = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity style={styles.gridItem} onPress={() => router.push(`/product/${item.id}`)}>
  ...
  const renderReel = ({ item }: { item: any }) => (
    <TouchableOpacity style={styles.gridItem} onPress={() => router.push({ pathname: '/(tabs)/reels', params: { initialReelId: item.id } })}>
...
const styles = StyleSheet.create({
  ...
  gridItem: { width: ITEM_SIZE, height: ITEM_SIZE, borderWidth: 0.5, borderColor: '#fff' },
